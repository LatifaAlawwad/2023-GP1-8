...
function validateMandatoryFields(form) {
    const mandatoryFields = ['placeName', 'city', 'neighbourhood', 'placeType', 'description'];
 ...
 try {
         const docRef = await addDoc(collection(db, 'ApprovedPlacesWeb'), placeData);
         console.log("Document written with ID: ", docRef.id);



         const uuid = uuidv4();
       await updateDoc(doc(db, 'ApprovedPlacesWeb', docRef.id), { place_id: uuid });
        ...
        fetch('areas.json')
            ...
            .then(data => {
                ...
                neighborhoods.forEach(neighborhood => {
                    ...
                    option.textContent = neighborhood.name_ar; // or name_en for English names
