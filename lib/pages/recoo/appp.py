...
# Function to recommend places based on item similarity
# Function to recommend places based on item similarity
def model(user_favorites, item_profiles):
    recommendations = set()
    favorite_place_ids = {favorite['place_id'] for favorite in user_favorites}  # Set comprehension to store favorite place IDs

    # Check if the user has more than 300 favorites
    if len(user_favorites) > 300:
        user_favorites = user_favorites[:300]  # Limit to first 300 places
    ...
    # Filter out recommendations that are already in favorites
    recommendations = [(place_id, score) for place_id, score in recommendations if place_id not in favorite_place_ids]
...
def get_recommendations():
    ...
    recommendations = model(user_favorites, item_profiles)
